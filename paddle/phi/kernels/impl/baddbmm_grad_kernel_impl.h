...
void BaddbmmGradKernel(const Context& dev_ctx,
  ...
  if (input_grad) {
    ...
    auto eigen_dinput_shape =
        Array3(input_grad->dims()[0], input_grad->dims()[1], input_grad->dims()[2]);
  ...
  if (x_grad) {
    ...
    // x_grad = out_grad * y'. x_grad: B x M x K, out_grad : B x M x N, y : B x K x N
  ...
  if (y_grad) {
    ...
    // y_grad = x' * out_grad. y_grad: B x K x N, out_grad : B x M x N, x : B x M x K
    for (int i = 0; i < x.dims()[0]; ++i) {
      auto out_grad_slice = out_grad.Slice(i, i + 1);
